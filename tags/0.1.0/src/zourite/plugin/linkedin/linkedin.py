...
class LinkedInApi():
    ...
    def doApiPutRequest(self, url, body, access_token):
        ...
        oauth_request.sign_request(self.linkedin.sig_method, self.linkedin.consumer, access_token)
        self.linkedin.connection.request(oauth_request.http_method, url, body=body, headers=oauth_request.to_header())
...
class StatusApi(LinkedInApi):
    ...
    def clearMyStatus(self, access_token):
        ...
        '''
        status   = self.doApiDeleteRequest(self.GET_STATUS_SELF_URL, access_token)       
        # TODO check dom result
        pass
